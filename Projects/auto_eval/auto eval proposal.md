...
**Supersedes nothing.** Rollout sequencing is in [[auto eval rollout plan]]. The rest of the detail is in this folder: [[auto eval plan]] (the six
...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------- |
| **G1** | **Scoring** instances as a proxy of accuracy on unlabelled data.                                                                        | PPI plus a human-labelled sample                                          | **only with the label investment** (§3.1)               |
...
| **G3** | **Rank a review queue.** Order by scores and/or defect classes, so a coder or an RCA starts at the most-likely-wrong encounter          | only *rank* correlation with error, reported as lift over random ordering | **yes**, once calibrated on the labels we have          |
| **G4** | **Gate.** Feed abstention so low-confidence encounters route to review                                                                  | prospective silent-mode validation; compliance sign-off                   | **no** — out of scope for v1                            |
| **G5** | **Pre-flight a candidate on unlabelled data.** Some signal before feedback returns weeks later                                          | a stable, version-pinned detector and a stated MDE                        | **partly** — needs G1 calibrated first                  |
